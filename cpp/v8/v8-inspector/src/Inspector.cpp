...
Inspector::Inspector(const std::unique_ptr<v8::Platform>& platform, const v8::Local<v8::Context>& context, const int webSocketPort) {
  ...
  inspector_client_ = std::unique_ptr<V8InspectorClientImpl>(new V8InspectorClientImpl(platform, context_, std::bind(&Inspector::sendMessage, this, std::placeholders::_1), std::bind(&Inspector::waitForFrontendMessage, this)));
}
...
void Inspector::onMessage(const std::string& message) {
  ...
  if (!jsonObject.IsEmpty()) {
    std::string method = getPropertyFromJson(context_, jsonObject, "method");
...
bool Inspector::compileScript(const v8::Local<v8::String>& source, const std::string& filePath, v8::Local<v8::Script>& script, const v8::TryCatch& tryCatch) {
  ...
  v8::ScriptOrigin scriptOrigin = v8::ScriptOrigin(v8::String::NewFromUtf8(isolate_, ("file://" + filePath).c_str(), v8::NewStringType::kNormal).ToLocalChecked());
...
bool Inspector::executeScript(const v8::Local<v8::Script>& script, const v8::TryCatch& tryCatch) {
  auto result = script->Run(context_);
  return !tryCatch.HasCaught();
}
...
void Inspector::executeScripts() {
  for (const std::string& filePath : scripts) {
    ...
    v8::Local<v8::String> source = v8::String::NewFromUtf8(isolate_, fileContent.c_str(), v8::NewStringType::kNormal).ToLocalChecked();
