...
class DebugWrapper {
  constructor() {
    ...
    this.DebugEvent = { Break: 1, Exception: 2, AfterCompile: 3, CompileError: 4, OOM: 5 };

    // The different types of steps.
    this.StepAction = { StepOut: 0, StepNext: 1, StepIn: 2 };

    // A copy of the scope types from runtime-debug.cc.
    // NOTE: these constants should be backward-compatible, so
    // add new ones to the end of this list.
    this.ScopeType = { Global: 0, Local: 1, With: 2, Closure: 3, Catch: 4, Block: 5, Script: 6, Eval: 7, Module: 8 };

    // Types of exceptions that can be broken upon.
    this.ExceptionBreak = { Caught: 0, Uncaught: 1 };
  ...
  enable() {
    this.sendMessageForMethodChecked('Debugger.enable');
  }
  disable() {
    this.sendMessageForMethodChecked('Debugger.disable');
  }
  ...
  stepOver() {
    this.sendMessageForMethodChecked('Debugger.stepOver');
  }
  stepInto() {
    this.sendMessageForMethodChecked('Debugger.stepInto');
  }
  stepOut() {
    this.sendMessageForMethodChecked('Debugger.stepOut');
  }

  setBreakOnException() {
    this.sendMessageForMethodChecked('Debugger.setPauseOnExceptions', { state: 'all' });
  }

  clearBreakOnException() {
    const newState = this.isBreakOnUncaughtException() ? 'uncaught' : 'none';
    this.sendMessageForMethodChecked('Debugger.setPauseOnExceptions', { state: newState });
  }
  ...
  setBreakPoint(func, opt_line, opt_column, opt_condition) {
    ...
    const loc = %ScriptLocationFromLine2(scriptid, opt_line, opt_column, offset);
  ...
  clearBreakPoint(breakpoint) {
    ...
    const { msgid, msg } = this.createMessage('Debugger.removeBreakpoint', { breakpointId: breakid });
  ...
  debuggerFlags() {
    return { breakPointsActive: { setValue: enabled => this.setBreakPointsActive(enabled) } };
  }
  ...
  setBreakPointsActive(enabled) {
    const { msgid, msg } = this.createMessage('Debugger.setBreakpointsActive', { active: enabled });
    this.sendMessage(msg);
    this.takeReplyChecked(msgid);
  }
  ...
  generatorScope(gen, index) {
    ...
    const scopeObject = {
      ...
      propertyNames: () => Object.keys(details[kScopeDetailsObjectIndex]).map((key, _) => key)
    };
    return { scopeType: () => details[kScopeDetailsTypeIndex], scopeIndex: () => index, scopeObject: () => scopeObject, setVariableValue: setScopeVariableValue };
  }
  ...
  setBreakPointAtLocation(scriptid, loc, opt_condition) {
    const params = { location: { scriptId: scriptid.toString(), lineNumber: loc.line, columnNumber: loc.column }, condition: opt_condition };
  ...
  execStatePrepareStep(action) {
    switch (action) {
      case this.StepAction.StepOut:
        this.stepOut();
        break;
      case this.StepAction.StepNext:
        this.stepOver();
        break;
      case this.StepAction.StepIn:
        this.stepInto();
        break;
      default:
        %AbortJS('Unsupported StepAction');
        break;
    }
  }
  ...
  execStateScopeType(type) {
    switch (type) {
      ...
      case 'local':
        return this.ScopeType.Local;
      case 'with':
        return this.ScopeType.With;
      case 'closure':
        return this.ScopeType.Closure;
      case 'catch':
        return this.ScopeType.Catch;
      case 'block':
        return this.ScopeType.Block;
      case 'script':
        return this.ScopeType.Script;
      case 'eval':
        return this.ScopeType.Eval;
      case 'module':
        return this.ScopeType.Module;
      default:
        %AbortJS('Unexpected scope type');
    }
  }
  ...
  execStateScopeObjectProperty(serialized_scope, prop) {
    ...
    return { value: () => val, isUndefined: () => false };
  }
  ...
  execStateScopeObject(obj) {
    ...
    return {
      value: () => scope,
      property: prop => this.execStateScopeObjectProperty(serialized_scope, prop),
  ...
  setVariableValue(frame, scope_index, name, value) {
    ...
    const { msgid, msg } = this.createMessage('Debugger.setVariableValue', { callFrameId: frameid, scopeNumber: scope_index, variableName: name, newValue: { value: value } });
    this.sendMessage(msg);
    const reply = this.takeReplyChecked(msgid);
    if (reply.error) {
  ...
  execStateScope(frame, scope_index) {
    ...
    return {
      scopeType: () => this.execStateScopeType(scope.type),
      scopeIndex: () => scope_index,
      frameIndex: () => frame.callFrameId,
      scopeObject: () => this.execStateScopeObject(scope.object),
      setVariableValue: (name, value) => this.setVariableValue(frame, scope_index, name, value)
    };
  }
  ...
  getProperties(objectId) {
    const { msgid, msg } = this.createMessage('Runtime.getProperties', { objectId: objectId, ownProperties: true });
    this.sendMessage(msg);
    const reply = this.takeReplyChecked(msgid);
  ...
  reconstructRemoteObject(obj) {
    ...
    switch (obj.type) {
        ...
        switch (obj.subtype) {
          case 'error': {
            ...
            switch (obj.className) {
              case 'EvalError':
                throw new EvalError(desc);
              case 'RangeError':
                throw new RangeError(desc);
              case 'ReferenceError':
                throw new ReferenceError(desc);
              case 'SyntaxError':
                throw new SyntaxError(desc);
              case 'TypeError':
                throw new TypeError(desc);
              case 'URIError':
                throw new URIError(desc);
              default:
          ...
          case 'array': {
            const array = [];
            const props = this.propertiesToObject(this.getProperties(obj.objectId));
      ...
      case 'bigint': {
        assertEquals('n', obj.unserializableValue.charAt(obj.unserializableValue.length - 1));
    ...
    return { value: () => value, isUndefined: () => isUndefined, type: () => obj.type, className: () => obj.className };
  }

  evaluateOnCallFrame(frame, expr, throw_on_side_effect = false) {
    const frameid = frame.callFrameId;
    const { msgid, msg } = this.createMessage('Debugger.evaluateOnCallFrame', { callFrameId: frameid, expression: expr, throwOnSideEffect: throw_on_side_effect });
    this.sendMessage(msg);
    const reply = this.takeReplyChecked(msgid);
  ...
  execStateFrameRestart(frame) {
    const frameid = frame.callFrameId;
    const { msgid, msg } = this.createMessage('Debugger.restartFrame', { callFrameId: frameid });
    this.sendMessage(msg);
    this.takeReplyChecked(msgid);
  }
  ...
  execStateFrame(frame) {
    ...
    return {
      ...
      evaluate: (expr, throw_on_side_effect) => this.evaluateOnCallFrame(frame, expr, throw_on_side_effect),
  ...
  evaluateGlobal(expr, throw_on_side_effect) {
    const { msgid, msg } = this.createMessage('Runtime.evaluate', { expression: expr, throwOnSideEffect: throw_on_side_effect });
    this.sendMessage(msg);
    const reply = this.takeReplyChecked(msgid);
  ...
  eventDataScriptSource(id) {
    const { msgid, msg } = this.createMessage('Debugger.getScriptSource', { scriptId: String(id) });
    this.sendMessage(msg);
    const reply = this.takeReplyChecked(msgid);
  ...
  eventDataScriptSetSource(id, src) {
    const { msgid, msg } = this.createMessage('Debugger.setScriptSource', { scriptId: id, scriptSource: src });
    this.sendMessage(msg);
    this.takeReplyChecked(msgid);
  }
  ...
  eventDataScript(params) {
    ...
    return { id: () => id, name: () => name, source: () => this.eventDataScriptSource(params.scriptId), setSource: src => this.eventDataScriptSetSource(id, src) };
  }
  ...
  handleDebuggerPaused(message) {
    ...
    let execState = {
      ...
      evaluateGlobal: expr => this.evaluateGlobal(expr),
  ...
  handleDebuggerScriptParsed(message) {
    const params = message.params;
    let eventData = { scriptId: params.scriptId, script: () => this.eventDataScript(params), eventType: this.DebugEvent.AfterCompile };
    ...
    this.invokeListener(this.DebugEvent.AfterCompile, undefined, eventData, undefined);
  }

  handleDebuggerScriptFailedToParse(message) {
    const params = message.params;
    let eventData = { scriptId: params.scriptId, script: () => this.eventDataScript(params), eventType: this.DebugEvent.CompileError };

    // TODO(jgruber): Arguments as needed. Still completely missing exec_state,
    // and eventData used to contain the script mirror instead of its id.
    this.invokeListener(this.DebugEvent.CompileError, undefined, eventData, undefined);
  }
...
Object.defineProperty(debug, 'ScopeType', {
  get: function() {
