// 使用的 boost.coroutine1, 在 c++11 以及以后的版本不推荐使用
//
...
void do_session(websocket::stream<beast::tcp_stream>& ws, net::yield_context yield) {
  ...
  ws.set_option(websocket::stream_base::decorator([](websocket::response_type& res) { res.set(http::field::server, std::string(BOOST_BEAST_VERSION_STRING) + " websocket-server-coro"); }));
